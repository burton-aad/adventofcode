...
print_v = lambda *x: None

class Prog:
    valid_keys = [chr(ord('a')+x) for x in range(26)]
    STATE_RUN = 0
    STATE_STUCK = 1
    STATE_END = 2
    ...
    def load(self, code):
        self.inst = [x.strip().split() for x in code]
        self.pc = 0
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='AoC 2017 - Jour 18')
    ...
    if args.v:
        print_v = lambda *x: print(*x)

    p = Prog(0, lines)
    while p.recv_cnt == 0 and p.is_running():
        p.run()

    print("Part 1: Prog {} recv {}".format(p.pid, p.pipe[-1]))
    ...
    while True:
        ...
        if (not p0.is_running()) and (not p1.is_running()):
            ...
            if (not p0.is_running()) and (not p1.is_running()):
                break
    print("Part 2: prog 1 send count {}".format(p1.snd_cnt))
    print_v(p0)
    print_v(p1)
